...
if (!Premieres::checkById((int)$query['id'])) {
	Core::answer([
		'status'  => false,
		'text'    => 'Идентификатор не найден',
		'element' => '#poster'
	]);
}
...
$bind = [
	'title,s'            => Core::htmlClear($query['title'], true),
	'year_release,s'     => (int)$query['release'],
	'country_id,s'       => implode(',', $query['country']),
	'genre_id,s'         => implode(',', $query['genre']),
	'actors,s'           => Core::htmlClear($query['actors'], true),
	'age_restrictions,i' => (int)$query['age_restrictions'],
	'director,s'         => Core::htmlClear($query['director'], true),
	'description,s'      => Core::htmlClear($query['description'], true)
];
...
if ($_FILES['poster']['size'] > 0) {
	...
	$folder_premier = $path . D_S . $query['id'];

	unlink($folder_premier . D_S . $db->poster);

	if (!is_dir($folder_premier)) mkdir($folder_premier, 0777, true);

	Upload::send($_FILES, $folder_premier . D_S, null, null, $time, false);

	$bind = ['poster,s' => $new_name];
}

Premieres::update($bind, $query['id']);
